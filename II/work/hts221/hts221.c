			      ...
			      struct sensor_value *val)
{
	...
	if (chan == SENSOR_CHAN_AMBIENT_TEMP) {
		LOG_DBG("hts221_channel_get SENSOR_CHAN_AMBIENT_TEMP");
		...
		LOG_DBG("hts221_channel_get SENSOR_CHAN_HUMIDITY");
...
{
	LOG_DBG("REG hts221_sample_fetch");
...
{
	LOG_DBG("REG hts221_read_conversion_data TUU");
...
int hts221_init(const struct device *dev)
{
	...
	if (status < 0) {
		LOG_ERR("Failed to read conversion data. REAL INIT");
		return status;
	}
